...
if($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_REQUEST["PostAction"]) && check_bitrix_sessid())
{
	...
	if (!strlen(trim($_REQUEST['COMPANY'])))
		$arWarning[] = 'Введите название вашей компании, или свое имя';
...
*/
	if(count($arWarning)<=0 && $bDoSubscribe)
	{
		...
		{
			if (isset($_REQUEST['mid']) && isset($_REQUEST['mhash']) && CSubscribeHandlers::GetMailHash($_REQUEST['mid'])==$_REQUEST['mhash']) {
				$arFields['CONFIRMED'] = 'Y';
				$arFields['SEND_CONFIRM'] = 'N';

				if ($_REQUEST['pid']>0 && (!isset($_SESSION['SIBSCRIBE_POSTINGS']) ||
					is_array($_SESSION['SIBSCRIBE_POSTINGS']) && !in_array(intval($_REQUEST['pid']), $_SESSION['SIBSCRIBE_POSTINGS'])) &&
					CModule::IncludeModule('statistic')) {

					if (!isset($_SESSION['SIBSCRIBE_POSTINGS']))
						$_SESSION['SIBSCRIBE_POSTINGS'] = array();
					$_SESSION['SIBSCRIBE_POSTINGS'][] = intval($_REQUEST['pid']);
					CStatEvent::AddCurrent('sibscribe_postings', 'posting_'.intval($_REQUEST['pid']), $_REQUEST['mid']);
				}
			}
		...
		if($res)
		{
			if ($USER->IsAuthorized()) {
				$USER->Update($USER->GetID(), array('WORK_COMPANY' => $_REQUEST['COMPANY']));
			} else {
				$NAME = $DB->ForSQL($_REQUEST['COMPANY'], 255);
				$DB->Query("INSERT INTO b_klava_subscriptions_add VALUES(".intval($ID).", '$NAME')
...
if(strlen($_REQUEST["sf_EMAIL"]) > 0 || $ID > 0 || $USER->IsAuthorized())
{
	...
	if($arSubscription = $rsSubscription->GetNext()) {
		if ($USER->IsAuthorized()) {
			...
			$arSubscription['COMPANY'] = $arUser['WORK_COMPANY'];
		} else {
			$arInfo = $DB->Query("SELECT * FROM b_klava_subscriptions_add WHERE ID=".intval($arSubscription['ID']))->GetNext();
			$arSubscription['COMPANY'] = $arInfo['NAME'];
		}
...
if ($_SERVER['REQUEST_METHOD']!='POST' && isset($_GET['mid']))
	$arResult['SUBSCRIPTION']['EMAIL'] = htmlspecialchars($_GET['mid']);
