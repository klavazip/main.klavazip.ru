...
if( count($ar_ElementID) > 0 )
{
	...
	while($ob_Element = $rs_Element->GetNextElement())
	{
		...
		$s_Analogi = false;
		foreach ($ar_Propeties['CML2_TRAITS']['DESCRIPTION'] as $k => $s_Val)
		{
			if($s_Val == 'Аналоги (XML_ID)')
				$s_Analogi = $ar_Propeties['CML2_TRAITS']['VALUE'][$k];
		}
		
		if( strlen($s_Analogi) > 0 )
		{
			$ar_XMLID = explode(',', $s_Analogi); 
			
			$rs_Element1 = CIBlockElement::GetList(array(), array('IBLOCK_ID' => KlavaCatalog::IBLOCK_ID, 'XML_ID' => $ar_XMLID, '>=CATALOG_QUANTITY' => 1), false, false, array('ID'));
			$ar_Fields['IS_ANALOGI'] = ($rs_Element1->SelectedRowsCount() > 0);
		}	
