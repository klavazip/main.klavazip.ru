...
$arResult["IPROPERTY_VALUES"] = $ipropValues->getValues();
			...
			{
				if (trim($ar['DESCRIPTION']) == GetMessage($BXMCP_NAME.'_ANALOGI'))
				{
					$key_analog=$key;
				}
			}
			...
			if ($key_analog>=0)
			{
				$ar['VALUE']=$arResult['PROPERTIES']['CML2_TRAITS']['VALUE'][$key_analog];
								
				$arSelect = array('NAME','ID','DETAIL_PAGE_URL','CATALOG_QUANTITY');
				$arFilterAnalog = array('IBLOCK_ID'=>8,'XML_ID' => explode(',',$ar['VALUE']));
				
				$arFilterAnalog['>=CATALOG_QUANTITY'] = '1';
				
				$dbre = CIBlockElement::GetList(array(),$arFilterAnalog,false,array('nTopCount'=>$arParams['ELEMENT_COUNT']),$arSelect);
				while($are = $dbre->GetNext())
				{
					$are['PRICES'] = CPrice::GetBasePrice($are['ID']);
					$arResult['ANALOGS'][] = $are;
				}
			}

			$arResult['DETAIL_PAGE_TITLE'] = $arResult['NAME'];
			$rs_Section = CIBlockSection::GetList(array(), array('ID' => $arResult['IBLOCK_SECTION_ID'], 'IBLOCK_ID' => $arResult['IBLOCK_ID']), false, array('UF_NEW_TITLE'));
			if($ar_Section = $rs_Section->Fetch())
			{
				$arResult['DETAIL_PAGE_TITLE'] = str_replace(array('#SECTION_NAME#', '#NAME#'), array($ar_Section['NAME'], $arResult['NAME']), $ar_Section['UF_NEW_TITLE']);
			}
			
			//<?=(strlen(trim($TITLE_PREFIX)) > 0 ? str_replace(array('#NAME#'),array($arResult['NAME']),$TITLE_PREFIX) : $arResult["NAME"] );
...
$arDeclenTitle = array(
		...
		'Кабели' => 'Кабель',
		'Матрицы' => 'Матрица (экран)',
		'Переходники' => 'Переходник',
		'Корпусные детали' => 'Корпусная деталь',
		'Разъемы' => 'Разъем',
