	...
	if (is_array($YANDEX_EXPORT))
	{
		...
		foreach ($YANDEX_EXPORT as $ykey => $yvalue)
		{
			...
			while ($arAcc = $res->GetNext())
			{
				...
				$strTmpOff.= "<url>http://".$arAcc['SERVER_NAME'].urldecode(htmlspecialcharsbx($arAcc["~DETAIL_PAGE_URL"])).(strstr($arAcc['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."r1=<?echo \$strReferer1; ?>&amp;r2=<?echo \$strReferer2; ?></url>\n";
