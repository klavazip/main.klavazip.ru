...
if ((false == isset($_REQUEST['IBLOCK_ID'])) || (0 == strlen($_REQUEST['IBLOCK_ID'])))
{
	require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php");
...
{
	$strPerm = CIBlock::GetPermission($intIBlockID);
	if ($strPerm < 'R')
	{
		require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php");
...
if (!empty($arOffers['IBLOCK_ID']))
{
	...
	{
		$strPerm = CIBlock::GetPermission($intOfferIBlockID);
		if ($strPerm < 'R')
		{
			require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	if (!empty($_REQUEST['save']))
	{
		...
		$XML_DATA = array_merge(/*$_POST['XML_DATA']['common'], */$arTypeParams, $addParams);
...
<?
	die();
		...
		{
			?><select name="XML_DATA[<? echo htmlspecialchars($group)?>][<? echo htmlspecialchars($key)?>]">
			...
			{
				?><option value="<?=$arProp['ID']?>"<? echo ($value == $arProp['ID'] ? ' selected' : ''); ?>>[<?=htmlspecialchars($key)?>] <?=htmlspecialchars($arProp['NAME'])?></option><?
			}
			if (!empty($IBLOCK['OFFERS_PROPERTY']))
			{
		...
		{
			return '<input type="text" size="3" name="XML_DATA[PARAMS][UNIT_'.$intCount.']" value="'.htmlspecialchars($value).'">';
		}
...
<?
		foreach ($arTypesConfig as $key => $arConfig):
?>
				<div id="config_<?=htmlspecialchars($key)?>" style="padding: 10px; display: <? echo ($type == $key ? 'block' : 'none'); ?>;">
...
?>
						<tr>
						<td align="right"><?=htmlspecialchars(GetMessage('YANDEX_PROP_'.$prop))?>: </td>
						<td style="white-space: nowrap;"><?__yand_show_selector($key, $prop, $arIBlock, (isset($arTypeValues[$key][$prop]) ? $arTypeValues[$key][$prop] : ''))?>&nbsp;<small>(<?=htmlspecialchars($prop)?>)</small></td>
						</tr>
<?
			endforeach;
?>
...
?>
			...
			{
				?><option value="<? echo htmlspecialchars($key);?>" <? echo ($key == $arSKUExport['SKU_EXPORT_COND'] ? 'selected' : '');?>><? echo htmlspecialcharsex($value); ?></option><?
			}
			?></select><?
			if (!empty($arSelectOfferProps))
			{
				...
				<tr>
					...
					foreach ($arSelectOfferProps as &$intPropID)
					{
						...
						?><option value="<? echo htmlspecialchars($intPropID); ?>" <? echo $strSelected; ?>><? echo htmlspecialcharsex($arIBlock['OFFERS_PROPERTY'][$intPropID]['NAME']);?></option><?
					}
					?></select></td>
					...
					{
						?><option value="<? echo htmlspecialchars($key);?>" <? echo ($key == $arSKUExport['SKU_PROP_COND']['COND'] ? 'selected' : ''); ?>><? echo htmlspecialcharsex($value); ?></option><?
					}
					?></select></td>
					...
					foreach ($arSelectOfferProps as &$intPropID)
					{
						...
						if (!empty($arProp['VALUES']))
						{
							...
							{
								?><option value="<? echo htmlspecialchars($intValueID); ?>" <? echo (!empty($arSKUExport['SKU_PROP_COND']['VALUES']) && in_array($intValueID,$arSKUExport['SKU_PROP_COND']['VALUES']) ? 'selected' : ''); ?>><? echo htmlspecialcharsex($strValue); ?></option><?
							}
							?></select><?
						}
						else
						{
							if (!empty($arSKUExport['SKU_PROP_COND']['VALUES']))
							{
								...
								{
									?><input type="text" name="SKU_PROP_VALUE_<? echo $arProp['ID']?>[]" value="<? echo htmlspecialchars($strValue);?>"><br><?
								}
							}
...
<?
		...
		{
?>
			<option value="<?=htmlspecialchars($key)?>"<? echo ($strRate == $key ? ' selected' : ''); ?>>(<?=htmlspecialchars($key)?>) <?=htmlspecialchars($title)?></option>
<?
		}
?>
		</select></td>
		...
		?>
		<td>+<input type="text" size="3" id="CURRENCY_PLUS_<?=$strCurrency?>" name="CURRENCY_PLUS[<?=$strCurrency?>]"<? echo ($strRate == 'SITE' ? ' disabled="disabled"' : ''); ?> value="<? echo htmlspecialchars($strPlus); ?>" />%</td>
	</tr>
<?
	}
?>
