...
/****************************************************************/
function get_section_arr($id){
   CModule::IncludeModule("iblock");
   $arr_s = CIBlockSection::GetByID($id);
   if($sects = $arr_s ->GetNext()){$section_name=$sects['NAME'];}
   return $section_name;
}
/****************************************************************/
...
if (strlen($strExportErrorMessage)<=0)
{
	...
	while ($obElement = $db_acc->GetNextElement())
	{
		...
		*/
		...
		if ($minPrice <= 0 || in_array($arAcc["ID"],array(31504,31058)) ) continue; //отброс N154C2-L02 и LP101WSA (TL) (A1)
		...
		$arAcc["~DETAIL_PAGE_URL"]=str_replace('%2F', '/', $arAcc["~DETAIL_PAGE_URL"]);
			...
			switch ($key)
			{
				...
				case 'title':
					if (!is_array($XML_DATA) || !is_array($XML_DATA['XML_DATA']) || !$XML_DATA['XML_DATA'][$key])
					{
	                  ...
	                  CModule::IncludeModule("iblock");
							$arr_s = CIBlockSection::GetByID($arAcc["IBLOCK_SECTION_ID"]);
							if($sects = $arr_s->GetNext()){
								$section_name=get_section_arr($sects["IBLOCK_SECTION_ID"]);
							}					        
	
							$strTmpOff.= "<".$key.">".iconv("utf-8", "windows-1251", $section_name)." ".yandex_text2xml($arAcc["NAME"], true)."</".$key.">\n";
						}
					}
					else
