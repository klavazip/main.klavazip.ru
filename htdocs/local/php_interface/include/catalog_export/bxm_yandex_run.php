...
/** *************************************************** */
// если эти слова есть в названии товара, мы его пропускаем
$SKIP_GOODS_NAME_WORDS = ($SKIP_GOODS_NAME_WORDS ? $SKIP_GOODS_NAME_WORDS : '');
$SKIP_GS_NW_t = explode(',',$SKIP_GOODS_NAME_WORDS);
$SKIP_GS_NW = array();
for($i=0;$i<count($SKIP_GS_NW_t);$i++)
{
    if(strlen(trim($SKIP_GS_NW_t[$i])) > 0)
	   $SKIP_GS_NW[] = '%'.trim($SKIP_GS_NW_t[$i]).'%';
}
$IBLOCK_ID = intval($IBLOCK_ID);
$db_iblock = CIBlock::GetByID($IBLOCK_ID);

//////////////////////////////////////////////////////////
// общее описание для всех товаров
$PRODUCT_DESCRIPTION_ADD = ($PRODUCT_DESCRIPTION_ADD ? $PRODUCT_DESCRIPTION_ADD : '');
// 
$arSectionNames = array();
$dbResultSectNames = CIBlockSection::GetList(array(),array('IBLOCK_ID'=>$IBLOCK_ID),false,array(
    'ID',
    'IBLOCK_ID',
    'NAME',
    'UF_TITLE', // Title ????????
...
while($arSN = $dbResultSectNames->Fetch())
{
   $arSectionNames[$arSN['ID']] = array(
      'UF_TITLE' => ( $arSN['UF_TITLE'] == NULL ? '' : $arSN['UF_TITLE'] ),
      'UF_TITLE2' => ( $arSN['UF_TITLE2'] == NULL ? '' : $arSN['UF_TITLE2'] ),
      'UF_NEW_TITLE' => ( $arSN['UF_NEW_TITLE'] == NULL ? '#NAME# ' : $arSN['UF_NEW_TITLE'] ),
      'SECTION_NAME' =>  $arSN['NAME'],
   );
}
		...
		while ($obElement = $res->GetNextElement())
		{
			...
			if (is_array($XML_DATA['XML_DATA']))
			{
				$arAcc["PROPERTIES"] = $obElement->GetProperties();
                                $arAcc["BXM_PROPERTIES"] = $arAcc["PROPERTIES"];
			}
                        else
                        {
                            $arAcc["BXM_PROPERTIES"] = $obElement->GetProperties();
                        }
...
/*					$dbVAT = CCatalogProduct::GetVATInfo($arAcc['ID']);
			...
			$strTmpOff.= "<url>http://".$ar_iblock['SERVER_NAME'].urldecode(htmlspecialcharsbx($arAcc["~DETAIL_PAGE_URL"])).(strstr($arAcc['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."r1=<?echo \$strReferer1; ?>&amp;r2=<?echo \$strReferer2; ?></url>\n";
            ...
            /*
              echo '<pre>';
              print_r($arAcc["BXM_PROPERTIES"]['ANALOGS_NAMES']['VALUE']);
              echo '</pre>'; 
			...
			foreach ($arYandexFields as $key)
			{
				...
				case 'name':
					...
					break;
				case 'description':
                                    if(strlen($PRODUCT_DESCRIPTION_ADD) > 0)
                                    {
                                        if($arSectionNames[$CUR_SECTION])
                                        {
                                           $ename = str_replace(array('#NAME#','#SECTION_NAME#'),array($arAcc["NAME"],$arSectionNames[$CUR_SECTION]['NAME']),$arSectionNames[$CUR_SECTION]['UF_NEW_TITLE']);
                                        }
                                        else
                                        {
                                            $ename = $arAcc["NAME"];
                                        ...
                                        $adstr = str_replace( 
                                            array('#NAME#','#ANALOGI#','#PREVIEW_TEXT#'),
                                            array(
                                                $ename,
                                                (isset($arAcc["BXM_PROPERTIES"]['ANALOGS_NAMES']['VALUE']) && strlen($arAcc["BXM_PROPERTIES"]['ANALOGS_NAMES']['VALUE']) > 0 ? strip_tags(preg_replace_callback("'&[^;]*;'", "yandex_replace_special",' Аналоги: '.$arAcc["BXM_PROPERTIES"]['ANALOGS_NAMES']['VALUE'])) : '' ),
                                                ($arAcc["PREVIEW_TEXT_TYPE"]=="html" ? strip_tags(preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arAcc["~PREVIEW_TEXT"])): preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arAcc["~PREVIEW_TEXT"]))                         
                                            ),
                                            $PRODUCT_DESCRIPTION_ADD);    
                                               
                                        $strTmpOff .=	"<description>".TruncateText(yandex_text2xml($adstr, true), 512)."</description>\n";
                                    }
                                    else
                                    {
                                        $strTmpOff .=  "<description>".TruncateText(
                                            yandex_text2xml(
                                            (($arAcc["PREVIEW_TEXT_TYPE"]=="html"?strip_tags(preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arAcc["~PREVIEW_TEXT"])) : preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arAcc["~PREVIEW_TEXT"])))
                                            , true)
                                            ,
...
/*							$dbVAT = CCatalogProduct::GetVATInfo($arOfferItem['ID']);
				...
				if (!empty($arCurrentOffer) && !empty($arCurrentPrice))
				{
					...
					$strOfferYandex .= "<url>http://".$ar_iblock['SERVER_NAME'].urldecode(htmlspecialcharsbx($arOfferItem["~DETAIL_PAGE_URL"])).(strstr($arOfferItem['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."r1=<?echo \$strReferer1; ?>&amp;r2=<?echo \$strReferer2; ?></url>\n";
...
/*						$dbVAT = CCatalogProduct::GetVATInfo($arItem['ID']);
				...
				$strOfferYandex.= "<url>http://".$ar_iblock['SERVER_NAME'].urldecode(htmlspecialcharsbx($arItem["~DETAIL_PAGE_URL"])).(strstr($arItem['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."r1=<?echo \$strReferer1; ?>&amp;r2=<?echo \$strReferer2; ?></url>\n";
