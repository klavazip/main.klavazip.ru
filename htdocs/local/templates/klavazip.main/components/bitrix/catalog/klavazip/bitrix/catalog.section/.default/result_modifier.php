...
$rsSections = CIBlockSection::GetList(
		...
		), false, array("ID", "DEPTH_LEVEL", "SECTION_PAGE_URL")
);
...
foreach ($arResult["ITEMS"] as $key => $arElement) {
	...
	$arFileTmp = CFile::ResizeImageGet(
			$arElement["DETAIL_PICTURE"], array("width" => $arParams["DISPLAY_IMG_WIDTH"], "height" => $arParams["DISPLAY_IMG_HEIGHT"]), BX_RESIZE_IMAGE_PROPORTIONAL, true, $arFilter
	);
	...
	$urlSection = str_replace(
		array("#SECTION_ID#", "#SECTION_CODE#"), array($arSections[$section_id]["ID"], $arSections[$section_id]["CODE"]), $urlTemplate
	);

	$arResult["ITEMS"][$key]["DETAIL_PAGE_URL"] = CIBlock::ReplaceDetailUrl(
			$urlSection, $arElement, true, "E"
	);
    }
}
...
if (intval($arResult['IBLOCK_SECTION_ID']) > 0 || intval($arResult['ID']) > 0  ) {
    
    $idSection = ( intval($arResult['IBLOCK_SECTION_ID']) > 0 ?  $arResult['IBLOCK_SECTION_ID']  : $arResult['ID'] );
    
    $arParentSection = CIBlockSection::GetList(array(), array('ID' => $idSection  , 'IBLOCK_ID' => $arResult['IBLOCK_ID']), false, array('UF_NEW_TITLE'))->Fetch();
    
    if ($arParentSection !== false && !empty($arParentSection['UF_NEW_TITLE'])) 
    {
    	foreach ($arResult['ITEMS'] as $k => $arItem) 
        {
    	    $arResult['ITEMS'][$k]['NAME'] = str_replace(array('#NAME#', '#SECTION_NAME#'), array($arItem['NAME'], $arResult['NAME']), $arParentSection['UF_NEW_TITLE']);
    	}
    }
}
