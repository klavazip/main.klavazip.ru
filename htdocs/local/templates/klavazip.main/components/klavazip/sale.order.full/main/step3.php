...
<div class="oneBlockStepInf">
	<h4>Выберите способ доставки, с помощью которого должен быть доставлен ваш заказ, и нажмите кнопку "Далее"</h4>
</div>
...
<div class="oneBlockStepInf">
	...
	<? 
	foreach ($arResult["DELIVERY"] as $ar_Value)
	{
		?>
		<div class="oneLineInfStep">
			<p style="float: none; width: inherit; color: inherit;">
				<label style="float: none;">
					<input type="radio" id="ID_DELIVERY_ID_<?= $ar_Value["ID"] ?>" name="<?=$ar_Value["FIELD_NAME"]?>" value="<?= $ar_Value["ID"] ?>"<?if ($ar_Value["CHECKED"]=="Y") echo " checked";?>>
					<?=$ar_Value['NAME']?>
				</label>
			</p>
			<p style="color: #808080; padding-left: 25px; font-size: 11px;">
				<?=$ar_Value['PERIOD_TEXT']?> <br /> Стоимость <?=$ar_Value['PRICE_FORMATED']?>
			</p>
			<div class="clear"></div>
