...
<div class="oneBlockStepInf">
	<h4>Выберите способ оплаты, с помощью которого вы хотите оплатить заказ, и нажмите кнопку "Далее"</h4>
</div>

<div class="oneBlockStepInf">
	<? 
	foreach ($arResult["PAY_SYSTEM"] as $ar_Value)
	{
		?>
		<div class="oneLineInfStep">
			<p style="float: none; width: inherit; color: inherit;">
				<label style="float: none;">
					<input type="radio" id="ID_PAY_SYSTEM_ID_<?= $ar_Value["ID"] ?>" name="PAY_SYSTEM_ID" value="<?= $ar_Value["ID"] ?>"<?if ($ar_Value["CHECKED"]=="Y") echo " checked";?>>
					<?=$ar_Value['PSA_NAME']?>
				</label>
			</p>
			<p style="color: #808080; padding-left: 25px; font-size: 11px;">
				<?=$ar_Value['DESCRIPTION']?> 
			</p>
			<div class="clear"></div>
		</div>
		<?		
