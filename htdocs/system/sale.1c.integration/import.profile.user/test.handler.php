...
$s_LogPatch = $_SERVER['DOCUMENT_ROOT']."/system/sale.1c.integration/profile.user/logs/";
$s_TestXMlPatch = $_SERVER['DOCUMENT_ROOT']."/system/sale.1c.integration/profile.user/test.xml/";
...
//if( $data = KlavaIntegrationMain::xml($s_TestXMlPatch.'/xml1.xml') )
if( $data = KlavaIntegrationMain::xml('test.xml/xml1.xml') )
{
	...
	//arraytofile($ar_UserResult, $s_LogPatch.'ar_UserResult.txt', "ar_UserResult");
	//echo '<pre>', print_r($ar_UserResult).'</pre>'; die();
	...
	foreach ($ar_UserResult as $ar_Value)
	{
		if( strlen($ar_Value['Партнер']) > 0 )
		{
			...
			if($ar_Users = $rs_Users->NavNext(true, false))
			{
				...
				if( intval($i_UserType) > 0 )
				{
					...
					if(strlen($ar_Value['ФактическийАдрес']['НаселПункт']) > 0)
					...
					switch ($i_UserType)
					{
						...
						default: # Для 2 и 3
							...
							# INN KPP OGRN RASCHET_SCHET COMPANY_NAME UCITY USTREET UHOME UKORPUS UFLAT UINDEX BIK_BANK KS_BANK NAME_BANK CITY_BANK BANK
					...
					if( $i_ProfileID = KlavaUserProfile::getProfileID($ar_Value['Ссылка']) )
					{
						...
						if( $i_CurrentPersonalType == $i_UserType)
						{
							...
							{
								CSaleOrderUserPropsValue::Update($ar_PropertyValue['ID'], array('USER_PROPS_ID' => $i_ProfileID, 'ORDER_PROPS_ID' => $ar_PropertyValue['ORDER_PROPS_ID'], 'VALUE' => $ar_IntegratValue[$ar_PropertyValue['CODE']]));
							}
						}
						else
						{
		...
		//arraytofile($ar_Status, $s_LogPatch.'ar_Status.txt', "ar_Status");
